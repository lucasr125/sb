...
if ((game.PlaceId == 6403373529) or (game.PlaceId == 9015014224) or (game.PlaceId == 11520107397)) then
	...
	Tab:AddButton({Name="Rejoin Game",Callback=function()
		TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlr)
	end});
	...
	Tab3:AddButton({Name="Get Glove Alchemist",Callback=function()
		if ((game.Players.LocalPlayer.leaderstats.Glove.Value == "Plague") and (#game.Players:GetChildren() >= 10)) then
			...
			OrionLib:MakeNotification({Name="Error",Content="You don't have Plague equipped or the server doesn't have 10 players",Image="rbxassetid://7733658504",Time=5});
		end
	end});
...
game:GetService("TeleportService"):Teleport(6403373529)
		...
		if (game.Workspace:FindFirstChild("Gravestone")) then
			if (game.Players.LocalPlayer.PlayerGui:FindFirstChild("Kills")) then
				if ((tostring(game.Players.LocalPlayer.PlayerGui.Kills.Frame.TextLabel.Text) == "10")) then
					...
					OrionLib:MakeNotification({Name="Error",Content="Mhmm, looks like you don't have 10 kills yet.",Image="rbxassetid://7733658504",Time=2});
				end
			else
				OrionLib:MakeNotification({Name="Error",Content="It looks like your kills are broken.",Image="rbxassetid://7733658504",Time=2});
			end
		else
			OrionLib:MakeNotification({Name="Error",Content="This server does not have 'Gravestone'.",Image="rbxassetid://7733658504",Time=2});
		end
	end});
	...
	GetMegarock = Tab4:AddToggle({Name="Get Megarock V2",Default=false,Callback=function(Value)
	...
	GetFish = Tab4:AddToggle({Name="Get Fish V2",Default=false,Callback=function(Value)
		_G.GetFish = Value;
	...
	GetVoodoo = Tab4:AddToggle({Name="Get Voodoo V2",Default=false,Callback=function(Value)
		_G.GetVoodoo = Value;
	...
	GetBob = Tab3:AddToggle({Name="Get 'bob' V1",Default=false,Callback=function(Value)
	...
	FarmBob = Tab3:AddToggle({Name="Get 'bob' V2",Default=false,Callback=function(Value)
		AutoFarmbob = Value;
		if (game.Players.LocalPlayer.leaderstats.Glove.Value == "Replica") then
		...
		elseif (Value == true) then
			OrionLib:MakeNotification({Name="Error",Content="You don't have Replica glove equipped",Image="rbxassetid://7733658504",Time=5});
			wait(0.05);
			FarmBob:Set(false);
		end
	end});
	...
	Tab3:AddToggle({Name="Auto Slap Baller [ All Glove Farm ]",Default=false,Callback=function(Value)
	...
	FarmReplica = Tab3:AddToggle({Name="Auto Slap Replica",Default=false,Callback=function(Value)
		ReplicaFarm = Value;
		if ((game.Players.LocalPlayer.leaderstats.Glove.Value == "Replica") and (game.Players.LocalPlayer.Character.IsInDefaultArena.Value == true)) then
	...
	Tab3:AddToggle({Name="Jet Farm",Default=false,Callback=function(Value)
		Jetfarm = Value;
		while Jetfarm do
	...
	Tab3:AddToggle({Name="Phase Farm",Default=false,Callback=function(Value)
		Phasefarm = Value;
		while Phasefarm do
			for i, v in pairs(game.Workspace:GetChildren()) do
	...
	Tab3:AddToggle({Name="Auto Pick Up Mitten",Default=false,Callback=function(Value)
		Giftfarm = Value;
		while Giftfarm do
			for i, v in pairs(game.Workspace:GetChildren()) do
	...
	Tab6:AddToggle({Name="Auto Enter Arena",Default=false,Callback=function(Value)
		_G.AutoEnter = Value;
	...
	Tab6:AddToggle({Name="Auto Enter Default Only Arena",Default=false,Callback=function(Value)
	...
	AutoFarmBrick1 = Tab3:AddToggle({Name="AutoFarm Brick V1",Default=false,Callback=function(Value)
		Brickfarm = Value;
		...
		elseif (Value == true) then
			OrionLib:MakeNotification({Name="Error",Content="You don't have Brick glove equipped",Image="rbxassetid://7733658504",Time=5});
			wait(0.05);
			AutoFarmBrick1:Set(false);
		end
	end});
	AutoFarmBrick2 = Tab3:AddToggle({Name="AutoFarm Brick V2",Default=false,Callback=function(Value)
		Brickfarm = Value;
		if (game.Players.LocalPlayer.leaderstats.Glove.Value == "Brick") then
			while Brickfarm do
	...
	AutoTycoon = Tab3:AddToggle({Name="Get Tycoon",Default=false,Callback=function(Value)
		...
		elseif (Value == true) then
			OrionLib:MakeNotification({Name="Error",Content="The server doesn't have 7 players",Image="rbxassetid://7733658504",Time=5});
			wait(0.05);
	...
	Tab7:AddDropdown({Name="Teleport",Default="",Options={"Arena","Lobby","Brazil","Island Slapple","Plate","Tournament","Moai Island","Default Arena","Island 1","Island 2","Island 3","SafeSpotBox 1.0","SafeSpotBox 2.0","Bed","Go Deep Into The Ground"},Callback=function(Value)
		...
		elseif (Value == "SafeSpot Box V1") then
			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = workspace['SafeBox'].CFrame * CFrame.new(0, 5, 0);
		elseif (Value == "SafeSpo tBox V2") then
		...
		elseif (Value == "Go To The Ground") then
	...
	Tab7:AddDropdown({Name="Animation Combat",Default="",Options={"Skukuchi Attacker","Skukuchi Target","Bomb Throw","Bubble Shoot","Revolver","Ban Hammer","Bomb","Rocket Launcher","Rojo","Rojo Recoil","Thor", "Rob"},Callback=function(Value)
	...
	GloveTab:AddTextbox({Name="Select Player to Spam Rojo",Default="Username",TextDisappear=false,Callback=function(Value)
	...
	GloveTab:AddToggle({Name="Spam Rojo",Default=false,Callback=function(Value)
		if (Person == nil) then
			Person = game.Players.LocalPlayer.Name;
		end
		_G.RojoSpam = Value;
	...
	GloveTab:AddTextbox({Name="Select Player to Teleport",Default="Username",TextDisappear=false,Callback=function(Value)
		TeleportPlayer = Value;
	end});
	GloveTab:AddButton({Name="Teleport Player",Callback=function()
	...
	GloveTab:AddTextbox({Name="Select go to Player",Default="Username",TextDisappear=false,Callback=function(Value)
		_G.PlayerTeleport = Value;
	end});
	GloveTab:AddButton({Name="Go to Player",Callback=function()
	...
	SavePlayer = GloveTab:AddToggle({Name="Spam Guardian Angel",Default=false,Callback=function(Value)
	...
	GloveTab:AddDropdown({Name="Retro Ability",Default="Rocket Launcher",Options={"Rocket Launcher","Ban Hammer","Bomb"},Callback=function(Value)
		RetroAbility = Value;
	end});
	GloveTab:AddToggle({Name="Spam Retro",Default=false,Callback=function(Value)
		RetroSpam = Value;
		while RetroSpam do
	...
	Cancel = false;
	GloveTab:AddTextbox({Name="Select Player to Punish",Default="Username",TextDisappear=false,Callback=function(Value)
		_G.PunishPlayer = Value;
	end});
	GloveTab:AddButton({Name="Punish Player",Callback=function()
	...
	GloveTab:AddButton({Name="Cancel Punishment",Callback=function()
		Cancel = true;
		wait(0.1);
		Cancel = false;
	end});
	GloveTab:AddTextbox({Name="Select Player to Kill",Default="Username",TextDisappear=false,Callback=function(Value)
		_G.KillerPlayer = Value;
	end});
	GloveTab:AddButton({Name="Kill Player",Callback=function()
	...
	GloveTab:AddButton({Name="Kill Random Player",Callback=function()
		if (game.Players.LocalPlayer.leaderstats.Glove.Value == "Home Run") then
			OGL = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame;
			game:GetService("ReplicatedStorage").HomeRun:FireServer({start=true});
			wait(3.05);
	...
	GloveTab:AddSlider({Name = "Set Cloud Speed",Min = 0.1,Max = 1.2,Default = 0.5,Color = Color3.fromRGB(255,255,255),Increment = 0.1,ValueName = "Speed",Callback = function(Value)
		_G.SetSpeedCloud = Value;
	end});
	CloudSpeed = GloveTab:AddToggle({Name = "Enable Custom Cloud Speed",Default = false,Callback = function(Value)
		_G.CloudSpeed = Value
	...
	GloveTab:AddButton({Name="Kick Player",Callback=function()
	...
	Tab7:AddButton({Name="Free All Animations [ You chat /e Name Animation]",Callback=function()
	...
	Tab7:AddButton({Name="Remove Glove Cooldown",Callback=function()
	...
	InfInvis = Tab7:AddButton({Name="Infinite Invisibility",Callback=function()
	...
	AutoShukuchi = Tab7:AddToggle({Name="Auto Shukuchi",Default=false,Callback=function(Value)
	...
	Tab7:AddToggle({Name="Glove ESP",Default=false,Callback=function(Value)
		GloveESP = Value;
	...
	FullKinetic = Tab7:AddToggle({Name="Auto Full Kinetic",Default=false,Callback=function(Value)
		...
		if ((game.Players.LocalPlayer.leaderstats.Glove.Value == "Kinetic") and game.Players.LocalPlayer.Character:FindFirstChild("entered")) then
			while FullKineticSpam do
				local args = {[1]={Force=0,Direction=Vector3.new(0, 0.1, 0)}};
	...
	Tab7:AddToggle({Name="Auto Click Destroy Tycoon",Default=false,Callback=function(Value)
	...
	RhythmNote = Tab7:AddToggle({Name="Rhythm Note Spam + Auto Press [ Equip Rhythm ]",Default=false,Callback=function(Value)
	...
	Tab7:AddColorpicker({Name="Set Color Skin [ You Have Glove Gold ]",Default=Color3.fromRGB(255, 0, 0),Callback=function(Value)
		_G.skinColor = Value;
	end});
	ColorSkin = Tab7:AddToggle({Name="Auto Color Skin [ Glove gold ]",Default=false,Callback=function(Value)
		_G.GoldColor = Value;
	...
	RainBox = Tab7:AddToggle({Name="Auto Rainbow [ Glove gold ]",Default=false,Callback=function(Value)
		_G.Rainbow = Value;
		if (game.Players.LocalPlayer.leaderstats.Glove.Value == "Golden") then
			while _G.Rainbow do
				local randomnumber = math.random(1004, 1032);
				local args = {[1]=false,[2]=BrickColor.new(randomnumber)};
	...
	AntiCOD = Tab2:AddToggle({Name="Anti Cube Of Death",Default=false,Callback=function(Value)
		...
		elseif game.Workspace:FindFirstChild("the cube of death(i heard it kills)", 1) then
	...
	Tab7:AddToggle({Name="Invisible Reverse [ FE ]",Default=false,Callback=function(Value)
		Invis_Reverse = Value;
	...
	InfRever = Tab7:AddToggle({Name="Infinite Reverse",Default=false,Callback=function(Value)
		InfReverse = Value;
	...
	PingPong = Tab7:AddToggle({Name="Ping Pong Orbit",Default=false,Callback=function(Value)
		PingPongOrbit = Value;
	...
	Tab7:AddSlider({Name="Ping Pong Orbit Speed",Min=0,Max=100,Default=10,Color=Color3.fromRGB(255, 255, 255),Increment=1,ValueName="Speed",Callback=function(Value)
		OrbitSpeed = Value;
	end});
	Tab10:AddToggle({Name="All Glove AutoFarm Slap",Default=false,Callback=function(Value)
		allFarming = Value;
		if (Value == true) then
			for i, v in pairs(game.Workspace.DEATHBARRIER:GetChildren()) do
				if ((v.ClassName == "Part") and (v.Name == "BLOCK")) then
	...
	Tab10:AddToggle({Name="Ghost AutoFarm Slap",Default=false,Callback=function(Value)
		Farming = Value;
		if (Value == true) then
			game.ReplicatedStorage.Ghostinvisibilityactivated:FireServer();
			while Farming do
	...
	Tab8:AddButton({Name="Inf Jump",Callback=function()
	...
	Tab8:AddButton({Name="View Testing Server [ Good for glove leaking ] [ You Have Banned ]",Callback=function()
		local teleportFunc = queueonteleport or queue_on_teleport or (syn and syn.queue_on_teleport);
		if teleportFunc then
			teleportFunc([[
        if not game:IsLoaded() then
...
game:GetService("GuiService"):ClearError()
	...
	Tab8:AddButton({Name="View Testing Slap Royale Server",Callback=function()
		local teleportFunc = queueonteleport or queue_on_teleport or (syn and syn.queue_on_teleport);
		if teleportFunc then
			teleportFunc([[
        if not game:IsLoaded() then
...
game:GetService("GuiService"):ClearError()
	...
	Tab11:AddDropdown({Name="Glove Sound",Default="Ghost",Options={"Ghost","Thanos","Space","Golden","Hitman","Error Death [ All Glove ]","Zombie [ All Glove ]"},Callback=function(Value)
		GloveSound = Value;
	end});
	...
	Tab13:AddToggle({Name="Auto Spam Ability",Default=false,Callback=function(Value)
		On = Value;
		...
		while On and (game.Players.LocalPlayer.leaderstats.Glove.Value == "Track") do
			...
			repeat
				RandomPlayer = players[math.random(1, #players)];
			until RandomPlayer.Character.isInArena.Value == true 
	...
	Tab13:AddDropdown({Name="Ability Spam All Glove",Default="Null",Options={"Null","Rhythm Explosion"},Callback=function(Value)
		AbilitySpamAllGlove = Value;
	end});
	Tab13:AddToggle({Name="Spam Ability All Glove",Default=false,Callback=function(Value)
		SpamAbility = Value;
	...
	Tab14:AddButton({Name="Slap Royela",Callback=function()
		if (game.Players.LocalPlayer.leaderstats.Slaps.Value >= 1000) then
			game:GetService("TeleportService"):Teleport(9426795465);
		end
	end});
	Tab14:AddButton({Name="Slap Battles [ Only Killstreak ]",Callback=function()
		game:GetService("TeleportService"):Teleport(11520107397);
	end});
	Tab14:AddButton({Name="Slap Battles [ NO ONE SHOT GLOVE ]",Callback=function()
		game:GetService("TeleportService"):Teleport(9015014224);
	end});
	Tab14:AddButton({Name="Slap Battles",Callback=function()
		game:GetService("TeleportService"):Teleport(6403373529);
	end});
	Tab14:AddButton({Name="Serverhop",Callback=function()
		local serverList = {};
		for _, v in ipairs(game:GetService("HttpService"):JSONDecode(game:HttpGetAsync("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100")).data) do
			if (v.playing and (type(v) == "table") and (v.maxPlayers > v.playing) and (v.id ~= game.JobId)) then
				serverList[#serverList + 1] = v.id;
	...
	Tab9:AddTextbox({Name="Glove",Default="Glove Name",TextDisappear=false,Callback=function(Value)
		EpinGlove = Value;
	end});
	Tab9:AddButton({Name="Epin Glove",Callback=function()
		if (game.Players.LocalPlayer.Character:FindFirstChild("entered") == nil) then
	...
	Tab15:AddButton({Name="Copy Guy that exists",Callback=function()
		setclipboard("Guy that exists#1915");
	end});
	Tab15:AddButton({Name="Copy Join Discord R2O",Callback=function()
		setclipboard("https://discord.gg/zty372wma5");
	end});
	Tab15:AddButton({Name="Copy Join Discord Mundion Exploits",Callback=function()
		setclipboard("https://discord.gg/jZFEk7Vn");
	end});
	Tab15:AddButton({Name="Copy Subscribe Mundion Exploits",Callback=function()
		setclipboard("www.youtube.com/@MundionExploits");
	end});
	Tab15:AddButton({Name="Copy thanhdat4461 [ Baconscripter ]",Callback=function()
		setclipboard("thanhdat4461#1232");
	end});
	Tab15:AddButton({Name="Destroy GUI",Callback=function()
		OrionLib:Destroy();
	end});
...
elseif (game.PlaceId == 11828384869) then
	...
	local Tab = Window:MakeTab({Name="Bagdes",Icon="rbxassetid://4483345998",PremiumOnly=false});
	local Tab1 = Window:MakeTab({Name="Misc",Icon="rbxassetid://4483345998",PremiumOnly=false});
	...
	Tab:AddButton({Name="Get Chain (Needs 1k slaps)",Callback=function()
		if (game.Players.LocalPlayer.leaderstats.Slaps.Value >= 1000) then
			local teleportFunc = queueonteleport or queue_on_teleport or (syn and syn.queue_on_teleport);
			if teleportFunc then
				teleportFunc([[
...
game:GetService("TeleportService"):Teleport(6403373529)
	...
	Tab1:AddButton({Name="View Testing Server [ Good for glove leaking ] [ You Have Banned ]",Callback=function()
		local teleportFunc = queueonteleport or queue_on_teleport or (syn and syn.queue_on_teleport);
		if teleportFunc then
			teleportFunc([[
        if not game:IsLoaded() then
...
game:GetService("GuiService"):ClearError()
	...
	Tab1:AddButton({Name="View Testing Slap Royale Server",Callback=function()
		local teleportFunc = queueonteleport or queue_on_teleport or (syn and syn.queue_on_teleport);
		if teleportFunc then
			teleportFunc([[
        if not game:IsLoaded() then
...
elseif (game.PlaceId == 13833961666) then
	...
	gethui().Orion.Name = "OrionEdited";
	shared.gloveHitBob = {Killstreak=game.ReplicatedStorage.KSHit,Reaper=game.ReplicatedStorage.ReaperHit};
	local Tab = Window:MakeTab({Name="Combat",Icon="rbxassetid://4483345998",PremiumOnly=false});
	local Tab1 = Window:MakeTab({Name="Anti Void",Icon="rbxassetid://4483345998",PremiumOnly=false});
	Tab:AddToggle({Name="Dame Boss",Default=false,Save=true,Flag="DameBoss",Callback=function(Value)
	...
	Tab:AddToggle({Name="Auto Slap BobClone",Default=false,Save=true,Flag="AutoSlapBobClone",Callback=function(Value)
	...
	Tab:AddToggle({Name="Auto Click Tycoon",Default=false,Save=true,Flag="AutoTycoon",Callback=function(Value)
		_G.AutoTycoon = Value;
		while _G.AutoTycoon do
			for i, v in pairs(workspace:GetDescendants()) do
				if ((v.Name == "Click") and v:FindFirstChild("ClickDetector")) then
	...
	Tab:AddToggle({Name="Click Button Tycoon Big",Default=false,Callback=function(Value)
		if (Value == true) then
			for i, v in pairs(workspace:GetDescendants()) do
	...
	Tab:AddToggle({Name="Auto Spawn Rob",Default=false,Callback=function(Value)
