if (getgenv().Rogue_AlreadyLoaded ~= nil) then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Error",Text="Rogue Hub was already found running!",Duration=7});
	return;
else
	getgenv().Rogue_AlreadyLoaded = 0;
end
if ((game.PlaceId == 6403373529) or (game.PlaceId == 9015014224) or (game.PlaceId == 9431156611)) then
else
	return;
end
...
if teleportFunc then
	teleportFunc([[https://github.com/lucasr125/sb/blob/main/slapp%20battles%20rogue%20hub", true))()]]);
end
...
sound.Volume = 0.5;
local Config = {WindowName=(("Rogue Hub | " .. game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name) or "Slap Battles"),Color=Color3.fromRGB(242, 125, 20)};
...
getgenv().settings = {gloveExtend=false,extendOption="Meat Stick",autoClicker=false,autoToxic=false,walkSpeed=20,jumpPower=50,noRagdoll=false,noReaper=false,noTimestop=false,noRock=false,autoJoin=false,joinOption="Normal Arena",noVoid=false,auraSlap=false,auraOption="Legit",voidRainbow=false,voidForce=false,playerForce=false,fov=70,spamFart=false,spin=false,spinSpeed=10,autoEquip=false,wallNoclip=false,acidGod=false,lavaGod=false,walkSpeedTog=false,jumpPowerTog=false,walkJesus=false,candyFarm=false,invis=false,tpClick=false,walkSpeedKey="NONE",jumpPowerKey="NONE",slappleFarm=false};
...
localPlr.CharacterAdded:Connect(function()
	...
	if getgenv().settings.noRagdoll then
		if (localPlr.Character.HumanoidRootPart == nil) then
			return;
		end
		localPlr.Character.Ragdolled:GetPropertyChangedSignal("Value"):Connect(function()
			if ((localPlr.Character.HumanoidRootPart == nil) or (getgenv().settings.noRagdoll == false) or getgenv().slapFarm) then
				return;
			end
...
if ((game.PlaceId ~= 9431156611) and (game.PrivateServerId == "")) then
	...
	corn:AddToolTip("Farm's candy corns for you. ( NOT WORKING FOR NOW)");
	...
	slapple:AddToolTip("Auto farm's slapple gloves for you. ( free slaps )");
end
local autoclicker = playerSec:CreateToggle("Autoclicker", getgenv().settings.autoClicker or false, function(bool)
	getgenv().settings.autoClicker = bool;
end);
autoclicker:AddToolTip("Automatically clicks for you");
...
if (game.PlaceId ~= 9431156611) then
	local toxicTog = playerSec:CreateToggle("Auto Toxic", getgenv().settings.autoToxic or false, function(bool)
		...
		if getgenv().settings.autoToxic then
			localPlr.leaderstats.Slaps:GetPropertyChangedSignal("Value"):Connect(function()
				if (not getgenv().settings.autoToxic or getgenv().slapFarm) then
					return;
				end
	...
	toxicTog:AddToolTip("Automatically says a toxic phrase when you slap someone ( NOT WORKING FOR NOW )");
	localPlr.leaderstats.Slaps:GetPropertyChangedSignal("Value"):Connect(function()
	...
	if (getgenv().settings.noRagdoll and localPlr.Character:FindFirstChildOfClass("Humanoid")) then
		localPlr.Character.Ragdolled:GetPropertyChangedSignal("Value"):Connect(function()
			if localPlr.Character:FindFirstChild("HumanoidRootPart") then
				if ((localPlr.Character.HumanoidRootPart == nil) or (getgenv().settings.noRagdoll == false) or getgenv().slapFarm or isTping) then
					return;
				end
				local oldCFrame = localPlr.Character.HumanoidRootPart.CFrame;
				repeat
...
if (game.PlaceId ~= 9431156611) then
	...
	local noClipWall = playerSec:CreateToggle("Pusher Wall Noclip", getgenv().settings.wallNoclip or false, function(bool)
	...
	noClipWall:AddToolTip("Clip's through the pusher wall ability.");
	local noTimestop = playerSec:CreateToggle("Anti Timestop & Cutscenes", getgenv().settings.noTimestop or false, function(bool)
		getgenv().settings.noTimestop = bool;
	end);
	noTimestop:AddToolTip("Makes you walk during timestops and cutscenes.");
	local togInvis = playerSec:CreateToggle("Invisible (FE)", false, function(bool)
		...
		if (getgenv().settings.invis and isLoaded and (localPlr.Character:FindFirstChild("entered") ~= nil)) then
			...
			repeat
				wait();
			until (localPlr.Character ~= nil) and localPlr.Character:WaitForChild("HumanoidRootPart") and (localPlr.Character:FindFirstChild("entered") == nil) 
		end
		if getgenv().settings.invis then
			...
			elseif ((localPlr.leaderstats.Slaps.Value <= 666) and getgenv().settings.invis) then
				game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Error",Text="You don't have enough slaps for the ghost glove! ( 666 Slaps )",Duration=5});
				return;
			end
			wait(0.3);
			game:GetService("ReplicatedStorage").Ghostinvisibilityactivated:FireServer();
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Invisible Activated",Text="You can now equip any other glove you want!",Duration=5});
			game:GetService("Lighting"):WaitForChild("dimensioncolor"):Destroy();
		end
	end);
	togInvis:AddToolTip("Makes you invisible without having to use Ghost ( REQUIRES 666 SLAPS )");
...
if (game.PlaceId == 9431156611) then
	...
	jesussy:AddToolTip("Make you walk on water.");
end
...
togspeed:CreateKeybind(tostring(getgenv().settings.walkSpeedKey or "NONE"):gsub("Enum.KeyCode.", ""), function(key)
	...
	if (key == "NONE") then
		...
		getgenv().settings.walkSpeedKey = Enum.KeyCode[key];
	end
end);
...
togjump:CreateKeybind(tostring(getgenv().settings.jumpPowerKey or "NONE"):gsub("Enum.KeyCode.", ""), function(key)
	...
	if (key == "NONE") then
		...
		getgenv().settings.jumpPowerKey = Enum.KeyCode[key];
	end
end);
...
if (game.PlaceId ~= 9431156611) then
	local cubeGod = playerSec:CreateButton("Delete the Cube of Death", function()
		if (game:GetService("Workspace").Arena.CubeOfDeathArea:FindFirstChild("the cube of death(i heard it kills)") ~= nil) then
			...
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Error",Text="The cube of death is already deleted!",Duration=5});
		end
	end);
...
if (game.PlaceId == 9431156611) then
	local butPhase = playerSec:CreateButton("Phase", function()
		if ((localPlr.Character ~= nil) and not localPlr.Character:FindFirstChild("inMatch").Value) then
			...
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Error",Text="Phase only works when your in the lobby.",Duration=5});
		end
	end);
	...
	playerSec:CreateButton("Get All Items", function()
		if ((localPlr.Character ~= nil) and localPlr.Character:FindFirstChild("inMatch").Value) then
			...
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Error",Text="You aren't in the game yet! Or something else has happened.",Duration=5});
		end
	end);
	playerSec:CreateButton("Use All Items", function()
		if ((localPlr.Character ~= nil) and localPlr.Character:FindFirstChild("inMatch").Value) then
			...
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Error",Text="You aren't in the game yet! Or something else went wrong...",Duration=5});
		end
	end);
end
...
if (game.PlaceId == 9431156611) then
	name = "Kill All (PATCHED, DONT USE)";
end
local farmTog = gloveSec:CreateToggle(name, false, function(bool)
	...
	elseif (setfpscap and getgenv().slapFarm) then
		setfpscap(50);
...
if (game.PlaceId == 9431156611) then
	toolName = "Kills all the players in your server (ban warning from mods)";
end
...
localPlr:GetMouse().Button1Down:Connect(function()
	...
	elseif (getgenv().settings.autoEquip and not getgenv().slapFarm and localPlr.Character:FindFirstChild("inMatch").Value and (getBackpackTool() ~= nil)) then
		localPlr.Character.Humanoid:EquipTool(getBackpackTool());
		getTool():Activate();
	end
end);
gloveSec:CreateToggle("Glove Extender", getgenv().settings, function(bool)
...
if (game.PlaceId ~= 9431156611) then
	...
	local joinRoyale = joinSec:CreateButton("Join Slap Royale", function()
		if (localPlr.leaderstats.Slaps.Value >= 1000) then
			game:GetService("TeleportService"):Teleport(9426795465);
		else
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Error",Text="You don't have 1000 slaps.",Duration=5});
		end
	end);
	joinRoyale:AddToolTip("Joins slap royale for you");
end
...
local auraDrop = auraSec:CreateDropdown("Type", {"Legit","Blatant"}, function(option)
	...
	if (getgenv().settings.auraSlap and (getgenv().settings.auraOption == "Blatant")) then
		if (game.PlaceId ~= 9431156611) then
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Note",Text="Blatant Type only works on the default glove, use legit for any glove type.",Duration=5});
		else
			game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Note",Text="Blatant Type only works on the Pack-A-Punch glove, use legit for any glove type.",Duration=5});
		end
	end
end);
...
infoSec:CreateButton("Devil of Rogue Hub: Kitzoon#7750", function()
	setclipboard("q");
	game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Note",Text="Copied Kitzoon's discord username and tag to your clipboard.",Duration=5});
end);
infoSec:CreateButton("Blood sucking helper: Kyron#6083", function()
	setclipboard("q");
	game:GetService("StarterGui"):SetCore("SendNotification", {Title="Script Notification",Text="Copied Kyron's discord username and tag to your clipboard.",Duration=5});
end);
infoSec:CreateButton("Consider donating on PayPal!", function()
	setclipboard("q");
	game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Note",Text="Copied our PayPal donate page to your clipboard, donate any amount to it!",Duration=5});
end);
infoSec:CreateButton("Consider donating on Bitcoin!", function()
	setclipboard("q");
	game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Note",Text="Copied our Bitcoin address to your clipboard, donate any amount to it!",Duration=5});
end);
infoSec:CreateButton("Join us on discord!", function()
	setclipboard("q");
	game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Note",Text="Copied our discord server to your clipboard.",Duration=5});
end);
local gameSec = infoTab:CreateSection("Game Information");
local playtime = 0;
local playLabel = gameSec:CreateLabel("Playtime: " .. playtime .. "s");
if (game.PlaceId ~= 9431156611) then
...
isLoaded = true;
task.wait(5);
game:GetService("StarterGui"):SetCore("SendNotification", {Title="Rogue Hub Fact",Text="Rogue hub has over 3500+ lines of code!",Duration=7});
while wait(1) do
	playtime = playtime + 1;
