...
local Window = Bracket:Window({Name=game_name,Enabled=true,Color=Color3.new(1, 0.4, 0.4),Size=UDim2.new(0, 296, 0, 296),Position=UDim2.new(0.5, -248, 0.5, -248)});
do
  local HomeTab = Window:Tab({Name="Home"});
  do
    local HomeSection = HomeTab:Section({Name="Automatics",Side="Left"});
    do
      ...
      local NotifGravestone = HomeSection:Button({Name="Notificate Gravestone",Side="Left",Callback=function()
        repeat
          task.wait();
        until game.Workspace:FindFirstChild("Gravestone") 
      ...
      local GetHallowJack = HomeSection:Button({Name="Get Hallow Jack",Side="Left",Callback=function()
        ...
        elseif not LocalPlayer.PlayerGui:FindFirstChild("Kills") then
          Bracket:Notification({Title="Halloween!",Description="Your kills are broken, reset your character",Duration=10});
        elseif (LocalPlayer.PlayerGui.Kills.Frame.TextLabel.Text >= 10) then
  ...
  local AntiTab = Window:Tab({Name="Antis"});
  do
    local Anti1 = AntiTab:Section({Name="Anti 1",Side="Left"});
    do
    ...
    local Anti2 = AntiTab:Section({Name="Anti 2",Side="Right"});
    do
  ...
  do
    local GloveSection = CombatTab:Section({Name="Glove",Side="Left"});
    do
      ...
      local GodMode = GloveSection:Dropdown({Name="God Mode ( resets character )",Flag="GodMode",Side="Left",List={{Name="God Mode",Mode="Button",Value=false,Callback=function(Selected)
        ...
        repeat
          task.wait();
        until LocalPlayer.Character:FindFirstChildWhichIsA("Tool") or LocalPlayer.Backpack:FindFirstChildWhichIsA("Tool") 
        for i, v in pairs(LocalPlayer.Character:GetChildren()) do
      ...
      end},{Name="God Mode + Invisibility",Mode="Button",Value=false,Callback=function(Selected)
      ...
      local ExtenderType = GloveSection:Dropdown({Name="Extender Option",Flag="ExtenderType",Side="Left",List={{Name="Meat Stick",Mode="Button",Value=false,Callback=function(Selected)
        getgenv().settings.ExtendOption = "Meat Stick";
      end},{Name="Pancake",Mode="Button",Value=false,Callback=function(Selected)
        getgenv().settings.ExtendOption = "Pancake";
      end},{Name="Growth",Mode="Button",Value=false,Callback=function(Selected)
        getgenv().settings.ExtendOption = "Growth";
      end},{Name="North Korea Wall",Mode="Button",Value=false,Callback=function(Selected)
        getgenv().settings.ExtendOption = "North Korea Wall";
      end},{Name="Slight Extend",Mode="Button",Value=false,Callback=function(Selected)
    ...
    local AbilitySection = CombatTab:Section({Name="Ability Spam",Side="Right"});
    do
      ...
      local RetroAbility = AbilitySection:Dropdown({Name="Retro Option",Flag="RetroAbility",Side="Left",List={{Name="Ban Hammer",Mode="Button",Value=true,Callback=function(Selected)
        getgenv().settings.RetroOption = "Ban Hammer";
      end},{Name="Rocket Launcher",Mode="Button",Value=false,Callback=function(Selected)
        getgenv().settings.RetroOption = "Rocket Launcher";
      end},{Name="Bomb",Mode="Button",Value=false,Callback=function(Selected)
  ...
  do
    local SoundSpamSection = MiscTab:Section({Name="Sound Spam",Side="Left"});
    do
    ...
    local ExtraFuncSection = MiscTab:Section({Name="Gaming Features",Side="Right"});
    do
      ...
      local SelecArena = ExtraFuncSection:Dropdown({Name="Select Arena",Flag="SelecArena",Side="Left",List={{Name="Default Only",Mode="Button",Value=true,Callback=function(Selected)
        getgenv().settings.ArenaSelected = "Teleport2";
      end},{Name="Normal Arena",Mode="Button",Value=false,Callback=function(Selected)
        getgenv().settings.ArenaSelected = "Teleport1";
      end}}});
      local Teleports = ExtraFuncSection:Dropdown({Name="Teleports",Flag="Teleports",Side="Left",List={{Name="Safe Spot",Mode="Button",Value=false,Callback=function(Selected)
        LocalPlayer.Character.HumanoidRootPart.CFrame = workspace.Spot.CFrame * CFrame.new(0, 40, 0);
      end},{Name="Normal Arena",Mode="Button",Value=false,Callback=function(Selected)
        LocalPlayer.Character.HumanoidRootPart.CFrame = game.workspace.Origo.CFrame * CFrame.new(0, -5, 0);
      end},{Name="Default Arena",Mode="Button",Value=false,Callback=function(Selected)
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(120, 360, -3);
      end},{Name="Tournament",Mode="Button",Value=false,Callback=function(Selected)
        LocalPlayer.Character.HumanoidRootPart.CFrame = workspace.Battlearena.Arena.CFrame * CFrame.new(0, 10, 0);
      end},{Name="Moai Island",Mode="Button",Value=false,Callback=function(Selected)
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(215, -15.5, 0.5);
      end},{Name="Slapple Island",Mode="Button",Value=false,Callback=function(Selected)
        LocalPlayer.Character.HumanoidRootPart.CFrame = game.workspace.Arena.island5.Union.CFrame * CFrame.new(0, 3.25, 0);
      end},{Name="Plate",Mode="Button",Value=false,Callback=function(Selected)
      ...
      local SelfKnockback = ExtraFuncSection:Toggle({Name="Self Knockback ( ONLY KINETIC )",Flag="SelfKnockback",Side="Left",Value=false,Callback=function(Toggle_Bool)
        ...
        while getgenv().settings.SelfKnockback do
          if ((getGlove() == "Kinetic") and LocalPlayer.Character:FindFirstChild("entered")) then
            local settings = {Force=0,Direction=0,0.1,0};
  ...
  local LocalTab = Window:Tab({Name="Local"});
  do
    local CharacterMovement = LocalTab:Section({Name="Character Movement",Side="Left"});
    do
    ...
    local ESP = LocalTab:Section({Name="ESP",Side="Right"});
    do
  ...
  local BadgeTab = Window:Tab({Name="Badges"});
  do
    local BadgesSection = BadgeTab:Section({Name="Badges",Side="Left"});
    do
      ...
      local BobFarm = BadgesSection:Toggle({Name="Bob Farm",Flag="BobFarm",Side="Left",Value=false,Callback=function(Toggle_Bool)
        ...
        if (getGlove() == "Replica") then
          while getgenv().settings.BobFarm do
            ...
            repeat
              task.wait();
            until LocalPlayer.Character:FindFirstChildWhichIsA("Tool") 
            keypress(69);
            keyrelease(69);
            game:GetService("ReplicatedStorage"):WaitForChild("HumanoidDied"):FireServer(LocalPlayer.Character, false);
            wait(3.5);
